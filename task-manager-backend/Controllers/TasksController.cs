...
public class TasksController : ControllerBase
{
    ...
    public async Task<ActionResult<IEnumerable<TaskItem>>> GetTasksByPriority(string priority)
    {
        try
        {
            var tasks = await _context.Tasks
    ...
    public async Task<ActionResult<IEnumerable<TaskItem>>> GetTasksByCategory(string category)
    {
        try
        {
            var tasks = await _context.Tasks
        ...
        {
            _logger.LogError(ex, "Error getting tasks by category");
            return StatusCode(500, new { message = "Internal server error", error = ex.Message });
        }
    }
}
